...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data } = await supabase
    ...
    .single();
  return {
    title: data?.title ? `${data.title} — Mnemis` : "Shared Meeting — Mnemis",
    description: "A shared meeting summary, powered by Mnemis AI.",
  };
}
    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="flex items-center justify-between h-[52px] px-6 border-b border-white/[0.06] bg-background/80 backdrop-blur-md">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-[13px] font-semibold text-white">Mnemis</span>
        </Link>
        <div className="flex items-center gap-4">
          ...
          <PublicReportButton
            itemId={t.id}
            reportType="share"
            className="h-8 px-3 rounded-md text-[12px] font-medium text-white/60 hover:text-white border border-white/[0.1] hover:bg-white/[0.05] transition-colors inline-flex items-center gap-2"
            variant="inline"
          />
        </div>
      </nav>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="rounded-[14px] border border-white/[0.07] bg-white/[0.02] p-6 text-center">
          <p className="text-[14px] text-white/50 mb-3">
            Powered by Mnemis — The AI Automation Workspace
          </p>
          <Link
            href="/login"
