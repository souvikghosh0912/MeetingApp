...
import { ModelType, Summary, TranscriptModelType, TranscriptSegment, Highlight, StudyPageSummary } from "@/types";
...
// ─── OCR via NVIDIA NIM Vision Model ────────────────────────────────────────
export async function performOCR(
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: `You are a precise OCR engine. Extract ALL text from this image EXACTLY as it appears, preserving the original structure, paragraphs, headings, bullet points, and formatting. Do NOT summarize, interpret, or add any commentary. Return ONLY the extracted text, nothing else.`,
          },
        ] as any,
      },
    ],
  ...
  const content = completion.choices[0]?.message?.content;
  if (!content) throw new Error("No response from OCR model");
  return content.trim();
}

// ─── Study Page Summarization via NVIDIA NIM ─────────────────────────────────
export async function summarizeStudyPage(
  extractedText: string,
  model: ModelType,
  summaryLength: "short" | "long",
  customInstructions?: string
): Promise<StudyPageSummary> {
  const client = getNimClient();

  const lengthInstruction =
    summaryLength === "short"
      ? "Be concise — max 5 key concepts, a 2-3 sentence summary, max 5 action items, max 5 definitions, max 5 facts."
      : "Be thorough — up to 10 key concepts, a detailed multi-paragraph summary, all action items, up to 10 definitions, and up to 10 facts.";

  const customInstructionBlock = customInstructions?.trim()
    ? `\n\n--- SPECIAL INSTRUCTIONS FROM USER ---\n${customInstructions.trim()}\n--- END OF SPECIAL INSTRUCTIONS ---`
    : "";

  // Detect bulk / multi-page content so the model knows to synthesize across pages
  const isMultiPage = extractedText.includes("--- Page ");
  const multiPageNote = isMultiPage
    ? "\n\nThis content spans multiple pages of study material. Synthesize across ALL pages to produce unified key_concepts, a coherent summary, comprehensive definitions, and representative facts. Do not treat pages in isolation."
    : "";

  const systemPrompt = `You are an expert study assistant and educational content analyst. Analyze the provided text from a book or study material and return a structured JSON summary.${customInstructionBlock}${multiPageNote}
...
Return ONLY valid JSON matching this exact schema:
{
  ...
  "summary": "string",
  "topics": ["string"],
  "action_items": [{"task": "string", "priority": "high|medium|low", "page_reference": "string or null"}],
...
Rules:
- key_concepts: Important terms, ideas, or concepts introduced in the text
- summary: A clear, educational summary of the content
- topics: Broader subject areas this content belongs to
- action_items: Specific study tasks, e.g. "Review the definition of X", "Practice Y", "Memorize Z"
- definitions: Explicit or implied definitions of key terms found in the text as {term, meaning} pairs
- facts: Standalone, verifiable factual statements extracted verbatim or closely paraphrased from the text
- difficulty: Estimate based on vocabulary and concept complexity
- estimated_read_minutes: How long it would take an average reader to study this material properly
- DO NOT include any text outside the JSON object`;
  ...
  const completion = await client.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Here is the extracted text from the study material:\n\n${extractedText}`,
      },
    ],
    temperature: 0.3,
    max_tokens: summaryLength === "short" ? 1024 : 3072,
    response_format: { type: "json_object" },
  });

  const content = completion.choices[0]?.message?.content;
  if (!content) throw new Error("No response from study summarization model");

  try {
    const cleaned = cleanJson(content);
    const parsed = JSON.parse(cleaned);
    return parsed as StudyPageSummary;
  } catch (err) {
    console.error("[study summarize] JSON Parse Error:", err, "Raw:", content);
    throw new Error("Failed to parse study summarization response as JSON");
  }
}

// ─── Flashcards Generation via NVIDIA NIM ────────────────────────────────────
export async function generateFlashcards(
  ...
  const systemPrompt = `You are an expert study assistant. Analyze the provided study material and generate a set of flashcards for spaced repetition learning.
...
Rules:
- Create 5 to 10 high-yield flashcards focusing on the most important concepts, terms, and facts.
- Keep the front concise (a question or a term).
- Keep the back clear and educational, but not overly long.
- DO NOT include any text outside the JSON object.`;
  ...
  const content = completion.choices[0]?.message?.content;
  if (!content) throw new Error("No response from flashcards model");
  ...
  try {
    ...
    console.error("JSON Parse Error when generating flashcards:", err, "Raw:", content);
