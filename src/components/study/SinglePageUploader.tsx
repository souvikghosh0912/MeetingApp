...
interface SinglePageUploaderProps {
  userId: string;
  plan?: Plan;
...
export function SinglePageUploader({
  userId,
  plan = "free",
  defaultModel = "gpt-oss-20b",
}: SinglePageUploaderProps) {
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], rejectedFiles: unknown[]) => {
      setError(null);
      const rejected = rejectedFiles as { errors: { code: string; message: string }[] }[];
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const ocrRes = await fetch("/api/study/ocr", { method: "POST", body: formData });
      if (!ocrRes.ok) {
        const data = await ocrRes.json();
        throw new Error(data.error ?? "Text extraction failed.");
      }
      const { extractedText } = await ocrRes.json();
      if (!extractedText?.trim()) throw new Error("No text detected in the image.");
  ...
  const isProcessing = ["uploading", "extracting", "summarizing"].includes(
    uploadState.status
  );
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div
        {...getRootProps()}
        className={cn(
          "relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed p-12 text-center cursor-pointer transition-colors duration-300 group",
          isDragActive
        ...
        {selectedFile ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/40 mt-0.5">{bytesToMb(selectedFile.size)} MB</p>
            </div>
          </div>
        ) : (
          <>
            ...
            >
              <Upload
                className={cn(
                  ...
                  isDragActive ? "text-violet-400" : "text-white/40 group-hover:text-violet-400"
                )}
              />
            </div>
      ...
      {/* Options — only shown after a file is selected */}
      {selectedFile && (
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <p className="mt-1.5 text-[10px] text-white/25">
              Optional — tells the AI how to approach this material.
            </p>
          </div>
