...
const MEETING_STEPS = [
  ...
  },
  {
    key: "summarizing",
    label: "Summarizing",
    description: "Generating your insights",
    icon: Sparkles,
    doneAt: 100,
  },
];
...
const STUDY_STEPS = [
  {
    ...
    description: "Securely uploading your book page",
    icon: Upload,
    doneAt: 35,
  },
  {
    ...
    description: "NVIDIA Vision is reading the page",
    icon: FileText,
    doneAt: 65,
  },
  {
    key: "summarizing",
    label: "Summarizing",
    description: "Generating study nodes and insights",
    icon: Sparkles,
    doneAt: 100,
  },
];

function getStepStatus(steps: any[], stepKey: string, currentStatus: string, progress: number) {
...
export function ProcessingStatus({ state, fileName, type = "meeting" }: ProcessingStatusProps) {
  const isDone = state.status === "done" || state.progress === 100;
  const steps = type === "meeting" ? MEETING_STEPS : STUDY_STEPS;
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 space-y-8 w-full">
      ...
      <div className="w-full max-w-sm space-y-3">
        {steps.map((step) => {
          const status = isDone ? "done" : getStepStatus(steps, step.key, state.status, state.progress);
      ...
      {isDone && (
        <p className="text-sm text-text-secondary animate-fade-in">
          Redirecting to your {type === "study" ? "study page" : "transcript"}...
        </p>
      )}
    </div>
  );
